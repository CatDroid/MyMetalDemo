...
kernel void
grayscaleKernel(texture2d<half, access::read>  inTexture  [[texture(AAPLTextureIndexInput)]], // 只读的纹理  纹理读 texture.read(uint2)
                texture2d<half, access::write> outTexture [[texture(AAPLTextureIndexOutput)]],// access::write 只写的纹理 纹理写 texture.write(float4, uint2)
                uint2                          gid        [[thread_position_in_grid]]) // 2维坐标 每个线程的网格坐标。
{
    // Check if the pixel is within the bounds of the output texture
    if((gid.x >= outTexture.get_width()) || (gid.y >= outTexture.get_height())) // 格子尺寸会超出图片尺寸 
    {
    ...
    half  gray     = dot(inColor.rgb, kRec709Luma); // = 0.2126 * r +  0.7152 * g + 0.0722 * b
