                                    ...
                                    metalKitTextureLoader:(nonnull MTKTextureLoader *)textureLoader
{
    ...
    for (MDLMaterialProperty *property in propertiesWithSemantic)
    {
           ...
           property.type == MDLMaterialPropertyTypeURL)
        {
            ...
            NSLog(@"property %@(type %lu): stringvalue=%@,sampler=%@", property.name,  (unsigned long)property.type, property.stringValue, property.textureSamplerValue);
            // name, MDLMaterialPropertyTypeURL : url or path , MDLTextureSampler
            
            // 增加多次检查
            NSError* error ;
            if (textureURL == nil || [textureURL checkResourceIsReachableAndReturnError:&error] == NO)
            {
                NSLog(@"%@ not exist with %@", textureURL, error);
                textureURL = [[NSBundle mainBundle] URLForResource:property.stringValue withExtension:nil];// 如果extension是nil 那么找到的是第一个名字匹配的
                NSLog(@"%@ try it 1 ", textureURL);
            }
            
            if (textureURL == nil || [textureURL checkResourceIsReachableAndReturnError:&error] == NO)
            {
                NSRange range = [property.stringValue rangeOfString:@"Map" options:NSBackwardsSearch];
                if (range.length > 0) {
                    // NSStringFromRange(range);
                    NSString* path = [property.stringValue substringToIndex: range.location];
                    NSLog(@"path is %@", path);
                    textureURL = [[NSBundle mainBundle] URLForResource:path withExtension:@"png"];
                    NSLog(@"%@ try it 2 ", textureURL);
                }
            }
            
            // NSAssert用于Objective-C，NSCAssert用于C语言中
            NSAssert([textureURL checkResourceIsReachableAndReturnError:&error],@"file not found %@", property.stringValue);
